...
### Chart Repo에 새로운 차트 추가

- demo 어플리케이션에 대한 Chart를 추가하기 위해 (`templates/`, `Chart.yaml`, `values.yaml`, `values-template.yaml`)파일을 복사합니다.

  ```
  cd ~/environment
  mkdir test/demo
  cp -r book-sample/helm/demo/* test/demo/
  ```

- Chart 레포지토리에 변경사항을 커밋 후 푸시합니다.

  ```
  ~/environment
  git add .
  git commit -m "Added demo chart"
  git push origin main
  ```

### ArgoCD Application 생성

- Application 탭으로 이동 후 `default` 프로젝트, `Manual` SYNC POLICY, 기존 설정한 Repository URL, `demo` Path, 기본 Cluster URL선택 후 Helm의 기본 values 파일에 `values.yaml`을 선택한 다음 ArgoCD App을 생성합니다.
- 
<img width="1292" alt="image" src="https://github.com/koDaegon/book-sample/assets/47220755/ad02281f-ab1e-4835-8203-b45219343060">

- Application 으로 이동하여 Sync를 맞춰줍니다.

### Buildspec 파일 수정

- 기존 Deploy 스테이지에서 수행하던 작업을 대신하여 values.yaml을 업데이트 할수 있도록 아래 명령을 스크립트에 추가 합니다.

```yaml
- |
  echo "### Update value to manifest repo"
  git clone https://git-codecommit.ap-northeast-2.amazonaws.com/v1/repos/chart-repo
  cd chart-repo
  cd demo
...
### ArgoCD를 통해 변경한 파이프라인 테스트

- 매니페스트 및 SSM Parameter Store에 저장된 컨테이너 이미지 태그가 동일한지 확인합니다.

  ArgoCD live manifest
  <img width="883" alt="image" src="https://github.com/koDaegon/book-sample/assets/47220755/b9f3c72d-63d6-430b-948f-eb5a813e8ce7">

  Parameter Store
  <img width="772" alt="image" src="https://github.com/koDaegon/book-sample/assets/47220755/05bfecce-06fe-41db-bbfc-8a774079efeb">



- 차트 레포에 Values.yaml을 수정하여 파이프라인이 잘 동작하는지 확인합니다.
